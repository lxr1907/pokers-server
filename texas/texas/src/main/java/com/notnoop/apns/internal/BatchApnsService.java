...
public class BatchApnsService extends AbstractApnsService {
	...
	// LoggerFactory.getLogger(BatchApnsService.class);

	/**
	 * How many seconds to wait for more messages before batch is send. Each
	 * message reset the wait time
	 *
	...
	/**
	 * How many seconds can be batch delayed before execution. This time is not
	 * exact amount after which the batch will run its roughly the time
	 */
	...
	public BatchApnsService(ApnsConnection prototype, ApnsFeedbackConnection feedback, int batchWaitTimeInSec,
			int maxBachWaitTimeInSec, ThreadFactory tf) {
		super(feedback);
			...
			try {
				...
				while ((msg = batch.poll()) != null) {
					try {
						...
						// logger.warn("Network exception sending message msg "
						// + msg.getIdentifier(), e);
					}
				}
			} finally {
